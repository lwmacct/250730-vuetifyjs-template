<template>
  <v-card variant="outlined">
    ...
    <v-card-text>
      <v-row>
        <v-col cols="12" sm="6">
          <v-switch :model-value="config.fixed" label="固定页脚模式" color="primary" hide-details
        ...
        <v-col cols="12" sm="6">
          <v-switch :model-value="showLongContent" label="显示长内容" color="primary" hide-details
        ...
        <v-col cols="12">
          <v-text-field :model-value="config.customText" label="自定义文本" placeholder="例如：© 2024 我的公司" variant="outlined"
            density="compact" @update:model-value="$emit('updateConfig', { customText: $event || undefined })" />
        </v-col>

        <v-col cols="12">
          <v-switch :model-value="config.showLinks" label="显示链接" color="primary" hide-details
